...
# Generator for 3D Point Clouds (Generating Sphere Points)
...
# Function to generate real spherical point clouds
def generate_sphere_points(radius, num_points):
    theta = torch.rand(num_points) * 2 * torch.pi  # Azimuthal angle
    phi = torch.acos(2 * torch.rand(num_points) - 1)  # Polar angle
    x = radius * torch.sin(phi) * torch.cos(theta)
    y = radius * torch.sin(phi) * torch.sin(theta)
    z = radius * torch.cos(phi)
    return torch.stack([x, y, z], dim=-1)  # Shape: (num_points, 3)
...
for epoch in range(num_epochs):
    for _ in range(batch_size):
        ...
        # Create real point clouds (sphere points)
        real_point_clouds = generate_sphere_points(radius=1.0, num_points=num_points).repeat(batch_size, 1, 1)
